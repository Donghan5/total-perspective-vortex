...
echo "Test Run: R$(printf "%02d" "$TEST_RUN")"
...
run_pipeline() {
    ...
    local train_log="${LOG_DIR}/S$(printf "%03d" "$SUBJECT_ID")_R$(printf "%02d" "$TEST_RUN")_${pipeline_name}_train.log"
    local predict_log="${LOG_DIR}/S$(printf "%03d" "$SUBJECT_ID")_R$(printf "%02d" "$TEST_RUN")_${pipeline_name}_predict.log"
